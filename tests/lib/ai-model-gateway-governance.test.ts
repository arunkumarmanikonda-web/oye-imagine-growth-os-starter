...
describe('governed model gateway', () => {
  it('uses the injected provider and records tenant-local cost', async () => {
    const result = await executeModelGateway({ tenantId: 'tenant-a', workspaceId: 'workspace-a', taskType: 'strategy', prompt: 'Build a craft-commerce strategy', maxCostUsd: 0.1 }, provider)
  ...
  it('does not share an explicit cache key across tenants', async () => {
    let calls = 0
    const countingProvider: ModelGatewayProviderHandler = async (request) => { calls += 1; return provider(request) }
    const common = { taskType: 'brand-qa', prompt: 'same prompt', cacheKey: 'same-explicit-key', maxCostUsd: 0.1 }
  ...
  it('blocks a provider result above the request cost ceiling', async () => {
    await expect(executeModelGateway({ tenantId: 'tenant-a', taskType: 'expensive-task', prompt: 'Do not execute above budget', maxCostUsd: 0.01 }, provider)).rejects.toThrow('cost cap exceeded')
  })
  ...
  it('keeps rule-based fallback explicit rather than defaulting it on in production', async () => {
    ...
    delete process.env.MODEL_GATEWAY_ALLOW_RULE_BASED_FALLBACK
    await expect(executeModelGateway({ tenantId: 'tenant-a', taskType: 'strategy', prompt: 'production request' })).rejects.toThrow()
  })
})
