...
function read(relativePath) {
  ...
  expect(fs.existsSync(absolute), `${relativePath}: required public intake route is missing.`)
...
expect(marketplace.includes("content-length"), 'Marketplace intake must reject oversized requests before persistence.')
...
expect(contact.includes('contentLength > 16_384'), 'Public contact must retain its 16 KiB request ceiling.')
...
console.log('Public intake abuse controls verified: marketplace and contact writes are payload-bounded, marketplace uses a honeypot, both use two-layer race-safe durable throttling, browser roles cannot call the limiter directly, and marketplace failures do not leak internal errors.')
