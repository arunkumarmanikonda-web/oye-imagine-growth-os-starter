...
-- Direct authenticated access is deliberately read-only. Audit internals and
-- mutation/idempotency records remain server-only even when RLS would match.
...
-- RLS policies intentionally use the membership helper from the authentication
-- hardening migration. It accepts either the stable control-plane tenant id or
-- the mapped operational UUID tenant id already stored on commercial rows.
drop policy if exists commercial_approval_requests_tenant_read on public.commercial_approval_requests;
create policy commercial_approval_requests_tenant_read
on public.commercial_approval_requests for select to authenticated
...
create policy commercial_contracts_tenant_read
...
create policy commercial_invoices_tenant_read
...
create policy commercial_ledger_entries_tenant_read
on public.commercial_ledger_entries for select to authenticated
using ((select private.current_user_has_tenant_membership(tenant_id)));

drop policy if exists commercial_media_balance_accounts_tenant_read on public.commercial_media_balance_accounts;
create policy commercial_media_balance_accounts_tenant_read
on public.commercial_media_balance_accounts for select to authenticated
using ((select private.current_user_has_tenant_membership(tenant_id)));

drop policy if exists commercial_subscriptions_tenant_read on public.commercial_subscriptions;
create policy commercial_subscriptions_tenant_read
on public.commercial_subscriptions for select to authenticated
using ((select private.current_user_has_tenant_membership(tenant_id)));

drop policy if exists commercial_tenant_snapshots_tenant_read on public.commercial_tenant_snapshots;
create policy commercial_tenant_snapshots_tenant_read
on public.commercial_tenant_snapshots for select to authenticated
using ((select private.current_user_has_tenant_membership(tenant_id)));

-- These two tables deliberately have no authenticated policies/grants.
-- They remain inaccessible through the ordinary client database API.
...
-- Privileged mutation functions are never callable from the public API by anon
-- or ordinary authenticated users. For the P0 hardening phase, server-side
...
-- Put trusted built-ins ahead of the exposed public schema. Existing function
-- bodies qualify commercial tables explicitly or are staged for regression test.
