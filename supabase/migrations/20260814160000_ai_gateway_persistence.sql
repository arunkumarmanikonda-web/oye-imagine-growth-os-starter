...
create index if not exists idx_ai_usage_ledger_tenant_created_at
  on public.ai_usage_ledger (tenant_id, created_at desc);
create index if not exists idx_ai_usage_ledger_tenant_task_created_at
  on public.ai_usage_ledger (tenant_id, task_type, created_at desc);
create index if not exists idx_ai_response_cache_tenant_expires
  on public.ai_response_cache (tenant_id, expires_at);
