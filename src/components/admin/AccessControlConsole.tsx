...
const emptySnapshot: Snapshot = { users: [], memberships: [], roles: [], overrides: [] }
...
export function AccessControlConsole() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setSnapshot({ users: body.users ?? [], memberships: body.memberships ?? [], roles: body.roles ?? [], overrides: body.overrides ?? [] })
      setSelectedUser((current) => current || body.users?.[0]?.id || '')
      setError(null)
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : 'Unable to load access control.')
    }
  }, [])
  ...
  async function createUser(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const body = await api('/api/admin/access/users', {
        method: 'POST',
        body: JSON.stringify({
          email: form.get('email'), fullName: form.get('fullName'), roleKey: form.get('roleKey'),
          tenantId: form.get('tenantId'), brandId: form.get('brandId'), workspaceId: form.get('workspaceId'),
          demoAccount: form.get('demoAccount') === 'on',
        }),
      })
      setIssuedCredential({ email: body.result.email, password: body.result.temporaryPassword })
  ...
  async function resetPassword() {
    ...
    try {
      ...
      setIssuedCredential({ email: selected.email ?? selected.id, password: body.result.temporaryPassword })
      await load()
    } catch (cause) { setError(cause instanceof Error ? cause.message : 'Password reset failed.') }
    finally { setBusy(false) }
  }

  async function setStatus(status: 'active' | 'suspended' | 'revoked') {
    if (!selected || !selectedMembership) return
    setBusy(true); setError(null)
    try {
      await api('/api/admin/access/users', { method: 'PATCH', body: JSON.stringify({ userId: selected.id, membershipId: selectedMembership.membership_id, status, reason: `Super Admin changed account status to ${status}.` }) })
      await load()
  ...
  async function deleteUser() {
    if (!selected || !selected.demoAccount) {
      setError('Deletion from this console is limited to accounts explicitly marked as disposable demo accounts.')
      return
    }
    if (!window.confirm(`Delete ${selected.email}? This permanently removes the disposable demo identity.`)) return
    setBusy(true); setError(null)
    try {
      await api('/api/admin/access/users', { method: 'DELETE', body: JSON.stringify({ userId: selected.id, reason: 'Super Admin deleted disposable demo account.' }) })
      setSelectedUser('')
      await load()
  ...
  async function addOverride(event: FormEvent<HTMLFormElement>) {
    ...
    setBusy(true); setError(null)
    try {
      await api('/api/admin/access/overrides', { method: 'POST', body: JSON.stringify({
        userId: selected.id, permissionKey: form.get('permissionKey'), effect: form.get('effect'), reason: form.get('reason'),
        tenantId: form.get('scope') === 'workspace' ? selectedMembership?.tenant_id : null,
        brandId: form.get('scope') === 'workspace' ? selectedMembership?.brand_id : null,
        workspaceId: form.get('scope') === 'workspace' ? selectedMembership?.workspace_id : null,
      }) })
  ...
  async function revokeOverride(overrideId: string) {
    setBusy(true); setError(null)
    try {
      await api('/api/admin/access/overrides', { method: 'DELETE', body: JSON.stringify({ overrideId, reason: 'Super Admin revoked the explicit override.' }) })
      await load()
  ...
  return (
    <div className="space-y-7">
      ...
      {issuedCredential ? <div className="rounded-[1.75rem] border border-amber-300/30 bg-amber-200/10 p-5"><p className="text-xs font-semibold uppercase tracking-[0.2em] text-amber-200">Show once · temporary credential</p><p className="mt-3 text-sm text-white">{issuedCredential.email}</p><code className="mt-2 block rounded-xl bg-black/40 p-3 text-lg text-amber-200">{issuedCredential.password}</code><p className="mt-3 text-xs text-white/55">The user must replace this password at first login. It is not stored in readable form.</p></div> : null}
      ...
      <section className="grid gap-6 xl:grid-cols-[1.05fr_1.6fr]">
        <form onSubmit={createUser} className="rounded-[2rem] border border-white/10 bg-white/[0.04] p-6">
          ...
          <div className="mt-5 grid gap-3">
            <input name="fullName" required placeholder="Full name" className="rounded-xl bg-black/30 px-4 py-3" />
            <input name="email" required type="email" placeholder="user@company.com" className="rounded-xl bg-black/30 px-4 py-3" />
            <select name="roleKey" required className="rounded-xl bg-black/30 px-4 py-3">{snapshot.roles.map((role) => <option key={role.role_key} value={role.role_key}>{role.role_name}</option>)}</select>
            <input name="tenantId" required defaultValue="tenant_oye_internal" placeholder="Tenant ID" className="rounded-xl bg-black/30 px-4 py-3" />
            <input name="brandId" required defaultValue="brand_oye_imagine" placeholder="Brand ID" className="rounded-xl bg-black/30 px-4 py-3" />
            <input name="workspaceId" required defaultValue="workspace_oye_internal" placeholder="Workspace ID" className="rounded-xl bg-black/30 px-4 py-3" />
            <label className="flex items-center gap-3 text-sm text-white/70"><input type="checkbox" name="demoAccount" /> Disposable demo account</label>
            <button disabled={busy} className="rounded-full bg-[#fdca5a] px-5 py-3 font-semibold text-black">Create user and temporary password</button>
          </div>
        </form>
        ...
        <section className="rounded-[2rem] border border-white/10 bg-white/[0.04] p-6">
          ...
          {selected ? <div className="mt-6 rounded-2xl bg-black/25 p-5"><div className="grid gap-3 md:grid-cols-2"><span><small className="block text-white/45">Name</small>{selected.fullName ?? '—'}</span><span><small className="block text-white/45">Role</small>{selectedMembership?.role_key ?? 'No active membership'}</span><span><small className="block text-white/45">Status</small>{selectedMembership?.status ?? '—'}</span><span><small className="block text-white/45">First-login reset</small>{selected.mustChangePassword ? 'Required' : 'Completed'}</span></div><div className="mt-5 flex flex-wrap gap-2"><button onClick={resetPassword} disabled={busy} className="rounded-full border border-white/15 px-4 py-2 text-sm">Issue new temporary password</button><button onClick={() => setStatus('active')} disabled={busy} className="rounded-full border border-white/15 px-4 py-2 text-sm">Activate</button><button onClick={() => setStatus('suspended')} disabled={busy} className="rounded-full border border-white/15 px-4 py-2 text-sm">Suspend</button>{selected.demoAccount ? <button onClick={deleteUser} disabled={busy} className="rounded-full border border-red-400/30 px-4 py-2 text-sm text-red-200">Delete demo</button> : null}</div></div> : null}
        </section>
      </section>
