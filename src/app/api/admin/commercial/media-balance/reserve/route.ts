...
export async function POST(request: NextRequest) {
  ...
  try {
    const raw = await reserveMediaBalanceRuntime({
      tenantId,
      amount,
      currency: typeof body.currency === 'string' && body.currency.trim() ? body.currency.trim() : 'INR',
      operationKey: buildOperationKey(body.operationKey, tenantId, 'reserve'),
    ...
    const message = error instanceof Error ? error.message : 'Failed to reserve media balance';
