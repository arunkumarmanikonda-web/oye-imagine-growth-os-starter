    ...
    <section className="oi-card rounded-[32px] p-8">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="max-w-3xl">
          ...
          <p className="mt-3 text-sm leading-6 text-slate-600">
            Canonical commercial-review, provider, activation, and continuity evidence derived from the shared bridge foundation.
          </p>
        </div>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-2">
        <section className="rounded-[24px] border border-slate-200 bg-white p-6">
          ...
          {evidence.sharedBlockers.length ? (
            <ul className="mt-3 space-y-2 text-sm text-slate-700">
              {evidence.sharedBlockers.map((item) => <li key={item}>{"• "}{item}</li>)}
            </ul>
        ...
        <section className="rounded-[24px] border border-slate-200 bg-white p-6">
          ...
          {evidence.continuityBlockers.length ? (
            <ul className="mt-3 space-y-2 text-sm text-slate-700">
              {evidence.continuityBlockers.map((item) => <li key={item}>{"• "}{item}</li>)}
            </ul>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-3">
        <section className="rounded-[24px] border border-slate-200 bg-white p-6">
          ...
          <ul className="mt-3 space-y-2 text-sm text-slate-700">
            {(evidence.commercialReviewBlockers.length ? evidence.commercialReviewBlockers : ["None"]).map((item) => <li key={`commercial-${item}`}>{"• "}{item}</li>)}
          </ul>
        </section>
        ...
        <section className="rounded-[24px] border border-slate-200 bg-white p-6">
          ...
          <ul className="mt-3 space-y-2 text-sm text-slate-700">
            {(evidence.providerReadinessBlockers.length ? evidence.providerReadinessBlockers : ["None"]).map((item) => <li key={`provider-${item}`}>{"• "}{item}</li>)}
          </ul>
        </section>
        ...
        <section className="rounded-[24px] border border-slate-200 bg-white p-6">
          ...
          <ul className="mt-3 space-y-2 text-sm text-slate-700">
            {(evidence.activationBlockers.length ? evidence.activationBlockers : ["None"]).map((item) => <li key={`activation-${item}`}>{"• "}{item}</li>)}
          </ul>
        </section>
      </div>
    </section>
