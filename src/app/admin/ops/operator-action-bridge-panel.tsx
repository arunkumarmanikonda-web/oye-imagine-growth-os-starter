...
export function OperatorActionBridgePanel({ companyName, bridge }: OperatorActionBridgePanelProps) {
    ...
    <section className="oi-card rounded-[32px] p-8">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <span className={toneClass(bridge.launchReady)}>Launch ready: {bridge.launchReady ? "yes" : "no"}</span>
          <span className={toneClass(bridge.managedQueueActionable)}>Queue actionable: {bridge.managedQueueActionable ? "yes" : "no"}</span>
        </div>
      </div>
      ...
      <section className="mt-6 rounded-[24px] border border-slate-200 bg-white p-6">
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
          {[
            ['Open approvals', bridge.queueSummary.openApprovals],
            ['Pending reports', bridge.queueSummary.pendingReports],
            ['Pending campaigns', bridge.queueSummary.pendingCampaigns],
            ['Pending strategy tasks', bridge.queueSummary.pendingStrategyTasks],
            ['Active blockers', bridge.queueSummary.activeBlockers],
          ].map(([label, value]) => <div key={String(label)} className="rounded-[20px] border border-slate-200 bg-slate-50 p-4"><div className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">{label}</div><div className="mt-2 text-2xl font-semibold text-slate-950">{value}</div></div>)}
        </div>
      </section>
