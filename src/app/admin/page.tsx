...
import { getOperatorAccessState } from '@/lib/admin-auth'
...
export default async function AdminPage() {
  const accessState = await getOperatorAccessState()
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-white">
      <div className="mx-auto max-w-7xl px-6 py-10">
        ...
        <h1 className="mt-5 text-5xl font-semibold leading-tight">Protected operator access with workspace truth</h1>
        <p className="mt-5 max-w-3xl text-lg leading-8 text-slate-300">
          The admin shell is now bound to lane-safe authentication, governed redirects, and canonical workspace selection.
        </p>
        ...
        <section className="mt-10 grid gap-6 lg:grid-cols-2">
          ...
          <div className="rounded-[2rem] border border-white/10 bg-white/5 p-6">
            <div className="text-xs uppercase tracking-[0.25em] text-cyan-300">Canonical truth</div>
            <div className="mt-4 space-y-3 text-sm text-slate-300">
              <div>Source: {neejeeTruth.canonicalTruthSource}</div>
              <div>Support email: {neejeeTruth.supportEmail}</div>
              <div>Deprecated sources removed: {neejeeTruth.deprecatedSources.join(', ')}</div>
            </div>
          </div>
        </section>

        <div className="mt-8 flex flex-wrap gap-3">
          <Link href="/admin/settings" className="rounded-full border border-white/20 px-5 py-3 font-medium text-white">
            Open settings
          </Link>
