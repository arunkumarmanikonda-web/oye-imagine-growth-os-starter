...
function providerBlockers(input: AutonomousExecutionInput) {
  ...
  if (input.actionKey === 'lifecycle.send') {
    ...
    if (input.channel === 'whatsapp' && provider !== 'whatsapp_cloud') return [`provider_channel_mismatch:${provider}:whatsapp`]
    if (input.channel === 'sms' && provider !== 'fast2sms') return [`provider_channel_mismatch:${provider}:sms`]
...
async function socialProviderGate(target: OperationalTarget, channel: string) {
  if (channel === 'youtube') {
    const account: any = await googleAccount(target)
    if (!account) return ['google_account_not_connected']
    const scopes = Array.isArray(account.scopes) ? account.scopes.map(String) : []
    const blockers: string[] = []
    if (!scopes.includes('https://www.googleapis.com/auth/youtube.upload')) blockers.push('youtube_upload_scope_missing')
    if (account.metadata?.discovery?.youtubeOk !== true) blockers.push('youtube_provider_discovery_not_verified')
    return blockers
  }
  if (!['facebook', 'instagram', 'linkedin'].includes(channel)) return [`social_channel_unsupported:${channel}`]
  try {
    await resolveSocialPublishingConnection(target, channel as 'facebook' | 'instagram' | 'linkedin')
    return []
  } catch (error) {
    return [error instanceof Error ? error.message.split(':')[0] : 'social_connection_missing']
  }
}
...
async function gate(access: ApiAccessContext, input: AutonomousExecutionInput): Promise<GateResult> {
  ...
  const admin = createSupabaseAdminClient()
  const [policyResult, routeResult, approvalResult] = await Promise.all([
  ...
  if (routeResult.error) throw new Error(`action_route_read_failed:${routeResult.error.message}`)
  if (approvalResult.error) throw new Error(`approval_policy_read_failed:${approvalResult.error.message}`)
  const policy: any = policyResult.data
  const route: any = routeResult.data
  const approvalPolicy: any = approvalResult.data
  const blockers: string[] = []
  if (!policy) blockers.push('growth_executor_policy_missing')
  else {
    if (!policy.enabled) blockers.push('growth_executor_disabled')
  ...
  else if (route.auto_execute_after_approval !== true) blockers.push('action_route_not_auto_executable')
  ...
  if (input.actionKey === 'campaign.launch') {
    ...
    if (account?.metadata?.discovery?.googleAdsOk !== true) blockers.push('google_ads_provider_discovery_not_verified')
  }
  if (input.actionKey === 'social.publish') blockers.push(...await socialProviderGate(target, input.channel))
  ...
  if (!readiness) blockers.push(`channel_qa_missing:${input.channel}`)
    ...
    if (!PASSING_QA_STATES.has(String(readiness.qa_status || '').toLowerCase())) blockers.push(`channel_qa_not_ready:${input.channel}:${readiness.qa_status}`)
    if (Array.isArray(readiness.blockers)) blockers.push(...readiness.blockers.map((value: unknown) => `channel_blocker:${String(value)}`))
  }
  return { ok: blockers.length === 0, blockers: Array.from(new Set(blockers)), policy, route, approvalPolicy, core, target }
}

async function existingRun(tenantId: string, key: string) {
  const { data, error } = await createSupabaseAdminClient().from('autonomous_execution_runs').select('*').eq('tenant_id', tenantId).eq('idempotency_key', key).maybeSingle()
...
async function createRun(access: ApiAccessContext, target: OperationalTarget, input: AutonomousExecutionInput) {
  ...
  const providerKey = input.actionKey === 'campaign.launch' || (input.actionKey === 'social.publish' && input.channel === 'youtube')
    ? 'google'
    : input.actionKey === 'lifecycle.send'
...
async function updateRun(id: string, patch: Record<string, unknown>) {
  const { data, error } = await createSupabaseAdminClient().from('autonomous_execution_runs').update({ ...patch, updated_at: new Date().toISOString() }).eq('run_id', id).select('*').single()
  if (error) throw new Error(`autonomous_run_update_failed:${error.message}`)
  return data
}

async function spendRpc(name: 'reserve_autonomous_media_spend' | 'settle_autonomous_media_spend' | 'release_autonomous_media_spend', id: string) {
  const { data, error } = await createSupabaseAdminClient().rpc(name, { p_run_id: id })
  if (error) throw new Error(`autonomous_spend_rpc_failed:${error.message}`)
  return data as any
}
...
async function machineApproval(access: ApiAccessContext, target: OperationalTarget, run: any, input: AutonomousExecutionInput) {
  ...
  const { data, error } = await createSupabaseAdminClient().from('commercial_approval_requests').upsert({
    approval_id: approvalId,
    tenant_id: target.tenantId,
    approval_type: approvalType,
    status: 'approved',
    payload: { actionKey: input.actionKey, resourceType: approvalType, externalAction: approvalType, autonomous: true, autonomyAgent: 'growth-executor', autonomousRunId: run.run_id, idempotencyKey: input.idempotencyKey, amount: run.requested_amount, currency: run.currency, channel: input.channel },
    resolution_payload: { decision: 'machine_scoped_approval', policyBasis: 'core_approval_policies + agent_autonomy_policies', approvedBy: access.subject, approvedAt: now },
...
async function executeCampaign(access: ApiAccessContext, input: AutonomousExecutionInput, gateResult: GateResult, run: any) {
  ...
  const providerCustomer = await readGoogleAdsCustomer(access, { workspaceId: input.workspaceId, customerId })
  if (providerCustomer.currencyCode !== currency) return markBlocked(run, [`provider_currency_mismatch:${providerCustomer.currencyCode}:${currency}`])
  const amount = Number(run.requested_amount)
  const micros = Math.round(amount * 1_000_000)
  if (amount <= 0 || !Number.isSafeInteger(micros) || micros <= 0) return markBlocked(run, ['prepaid_spend_amount_invalid'])
  const reservation = await spendRpc('reserve_autonomous_media_spend', run.run_id)
  ...
  try {
    ...
    const createdResourceName = String(created.resourceName)
    ...
    if (!settlement?.ok) {
      try {
        await setCampaignStatus(access, { workspaceId: input.workspaceId, approvalId: approval.approval_id, customerId, resourceName: createdResourceName, status: 'PAUSED' })
        await spendRpc('release_autonomous_media_spend', run.run_id)
      } catch {}
      return updateRun(run.run_id, { status: 'reconciliation_required', provider_result: { enabled, settlement }, error_code: String(settlement?.code || 'media_settlement_failed'), completed_at: new Date().toISOString() })
    }
    ...
    if (!enableAttempted) {
      await spendRpc('release_autonomous_media_spend', run.run_id).catch(() => null)
    ...
    if (resourceName) {
      try {
        const verification = await readCampaign(access, { workspaceId: input.workspaceId, customerId, resourceName })
        const state = String(verification?.campaign?.status || '').toUpperCase()
        if (state === 'ENABLED') {
          const settlement = await spendRpc('settle_autonomous_media_spend', run.run_id)
          return updateRun(run.run_id, settlement?.ok
            ? { status: 'succeeded', provider_result: { phase: 'enabled_verified_after_retry', verification, settlement }, error_code: null, completed_at: new Date().toISOString() }
            : { status: 'reconciliation_required', provider_result: { verification, settlement }, error_code: String(settlement?.code || code), completed_at: new Date().toISOString() })
        }
        if (state === 'PAUSED') {
          await spendRpc('release_autonomous_media_spend', run.run_id).catch(() => null)
...
async function executeLifecycle(access: ApiAccessContext, input: AutonomousExecutionInput, gateResult: GateResult, run: any) {
  const payload = input.payload || {}
  if (!payload.purpose || !payload.subject) return markBlocked(run, ['lifecycle_payload_incomplete'])
  const approval: any = await machineApproval(access, gateResult.target, run, input)
  ...
  try {
    ...
    return updateRun(run.run_id, { status: 'failed', error_code: error instanceof Error ? error.message.split(':')[0] : 'autonomous_lifecycle_failed', completed_at: new Date().toISOString() })
  }
}

async function executeSocial(access: ApiAccessContext, input: AutonomousExecutionInput, gateResult: GateResult, run: any) {
  const payload = input.payload || {}
  const approval: any = await machineApproval(access, gateResult.target, run, input)
  await updateRun(run.run_id, { status: 'executing', approval_id: approval.approval_id, started_at: new Date().toISOString(), blockers: [] })
  try {
    const result = input.channel === 'youtube'
      ? await publishYouTubeVideo(access, {
      ...
      : await publishSocialContent(access, {
          workspaceId: input.workspaceId,
          channel: input.channel as 'facebook' | 'instagram' | 'linkedin',
          text: payload.text ? String(payload.text) : undefined,
          mediaUrl: payload.mediaUrl ? String(payload.mediaUrl) : undefined,
          mediaType: payload.mediaType,
          linkUrl: payload.linkUrl ? String(payload.linkUrl) : undefined,
          title: payload.title ? String(payload.title) : undefined,
        })
    return updateRun(run.run_id, { status: 'succeeded', provider_result: result, external_resource_id: result.resourceId || null, error_code: null, completed_at: new Date().toISOString() })
  } catch (error) {
    const code = error instanceof Error ? error.message.split(':')[0] : 'autonomous_social_publish_failed'
    const ambiguous = /verification|post_id_missing|media_id_missing|upload_failed|privacy_verification/.test(code)
    return updateRun(run.run_id, { status: ambiguous ? 'verification_pending' : 'failed', error_code: code, completed_at: ambiguous ? null : new Date().toISOString() })
  }
}
...
export async function autonomousExecutionStatus(access: ApiAccessContext, workspaceId?: string) {
  ...
  const [policy, media, readiness, runs, routes, approvals, google, socialAccounts] = await Promise.all([
    ...
    (() => {
      let query = admin.from('integration_accounts').select('id,provider,status,metadata,last_verified_at').eq('tenant_id', target.tenantId).in('provider', ['meta', 'linkedin']).eq('status', 'connected')
      if (target.workspaceId) query = query.eq('workspace_id', target.workspaceId)
  ...
  for (const row of readiness.data || []) if (!latest.has(row.channel)) latest.set(row.channel, row)
  const googleRow: any = google
  const discovery = googleRow?.metadata?.discovery
  const socialRows: any[] = socialAccounts.data || []
  const meta = socialRows.find(row => row.provider === 'meta')
  const linkedin = socialRows.find(row => row.provider === 'linkedin')
  const googleScopes = Array.isArray(googleRow?.scopes) ? googleRow.scopes.map(String) : []
  return {
    target,
    core,
    ...
    autonomousApprovalPolicies: approvals.data || [],
...
export async function setAutonomyKillSwitch(access: ApiAccessContext, input: { workspaceId?: string; active: boolean }) {
  ...
  const admin = createSupabaseAdminClient()
  const now = new Date().toISOString()
  const { data, error } = await admin.from('agent_autonomy_policies').update({ kill_switch: input.active, updated_at: now }).eq('tenant_id', core.tenantId).eq('workspace_id', core.workspaceId).eq('agent_key', 'growth-executor').select('*').single()
...
export async function recordChannelReadiness(access: ApiAccessContext, input: { workspaceId?: string; channel: string; ready: boolean; blockers?: string[]; note?: string }) {
  ...
  const { data, error } = await createSupabaseAdminClient().from('execution_channel_publish_readiness').insert({
