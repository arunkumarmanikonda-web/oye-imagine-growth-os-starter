...
async function googleOAuthRuntimeConfig(): Promise<GoogleOAuthRuntimeConfig> {
  const resolution = await resolveRuntimeProviderFields({
    providerKey: 'google_oauth',
    fieldKeys: ['GOOGLE_CLIENT_ID', 'GOOGLE_CLIENT_SECRET', 'GOOGLE_OAUTH_REDIRECT_URI'],
  })
  const clientId = resolution.values.GOOGLE_CLIENT_ID?.trim()
  const clientSecret = resolution.values.GOOGLE_CLIENT_SECRET?.trim()
  if (!clientId || !clientSecret) throw new Error('google_oauth_not_configured')
  const configuredRedirect = resolution.values.GOOGLE_OAUTH_REDIRECT_URI?.trim()
  const redirectUri = configuredRedirect || `${(process.env.NEXT_PUBLIC_SITE_URL || 'https://www.oyeimagine.com').replace(/\/$/, '')}/api/public/integrations/google/callback`
  return {
    clientId,
    clientSecret,
    redirectUri,
    sources: {
      GOOGLE_CLIENT_ID: resolution.sources.GOOGLE_CLIENT_ID,
      GOOGLE_CLIENT_SECRET: resolution.sources.GOOGLE_CLIENT_SECRET,
      GOOGLE_OAUTH_REDIRECT_URI: configuredRedirect ? resolution.sources.GOOGLE_OAUTH_REDIRECT_URI : 'derived',
    },
  }
}

function keyMaterial() {
  const raw = requiredEnvironment('OYE_OAUTH_ENCRYPTION_KEY')
...
function stateSecret() {
  return process.env.OYE_OAUTH_STATE_SECRET?.trim() || requiredEnvironment('OYE_OAUTH_ENCRYPTION_KEY')
}
...
export function verifyGoogleOAuthState(state: string) {
  ...
  if (suppliedBuffer.length !== expectedBuffer.length || !crypto.timingSafeEqual(suppliedBuffer, expectedBuffer)) throw new Error('oauth_state_invalid')
...
export async function googleAuthorizationUrl(access: ApiAccessContext, requestedWorkspaceId?: string) {
  ...
  const config = await googleOAuthRuntimeConfig()
  ...
  return {
    ...
    tenantId: target.tenantId,
    workspaceId: target.workspaceId,
    configurationSource: config.sources,
  }
}
...
export async function discoverGoogleResources(tenantId: string, workspaceId?: string | null) {
  ...
  const [ads, analytics, search, youtube] = await Promise.allSettled([
    ...
    fetch('https://www.googleapis.com/youtube/v3/channels?part=id,snippet&mine=true', { headers }).then(async response => ({ ok: response.ok, status: response.status, data: await response.json().catch(() => ({})) })),
  ])
  const value = (result: PromiseSettledResult<any>) => result.status === 'fulfilled' ? result.value : { ok: false, status: 0, data: { error: 'request_failed' } }
